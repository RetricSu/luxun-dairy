...
# URL of the Lu Xun diary for 1920
URL = "https://zh.wikisource.org/zh-hans/%E9%AD%AF%E8%BF%85%E6%97%A5%E8%A8%98/%E6%97%A5%E8%A8%98%E7%AC%AC%E4%B9%9D"
...
def scrape_luxun_diary():
    ...
    for heading in month_headings:
        ...
        for elem in content_elements:
            ...
            # Create ISO date
            iso_date = f"1920-{current_month_num:02d}-{current_day:02d}"
            
            # Create raw date
            day_text = get_chinese_number(current_day) + "日"
            date_raw = f"一九二〇年{current_month}{day_text}"
    ...
    for month, entries in all_months_entries.items():
        if entries:
            months_count[f"一九二〇年{month}"] = len(entries)
    
    for month, count in sorted(months_count.items(), key=lambda x: month_map.get(x[0].replace("一九二〇年", ""), 0)):
...
def save_common_diary_json(items):
    ...
    os.makedirs("output_1920", exist_ok=True)
    
    # Save as a single JSON file
    output_file = "output_1920/luxun_nine_diary.json"
    ...
    individual_dir = "output_1920/individual_entries"
...
if __name__ == "__main__":
    print("Starting to scrape Lu Xun's 1920 diary...")
