...
# URL of the Lu Xun diary for 1915
URL = "https://zh.wikisource.org/zh-hans/%E9%AD%AF%E8%BF%85%E6%97%A5%E8%A8%98/%E4%B9%99%E5%8D%AF%E6%97%A5%E8%A8%98"
...
def scrape_luxun_diary():
    ...
    for heading in month_headings:
        ...
        while next_sibling:
            if next_sibling.name == 'pre':
                pre_elements.append(next_sibling)
            elif next_sibling.name == 'div' and next_sibling.find('h2'):
                break
            next_sibling = next_sibling.next_sibling
        
        print(f"  Found {len(pre_elements)} diary entry blocks")
        ...
        for pre in pre_elements:
            ...
            day_match = re.search(r'^([一二三四五六七八九十]+日)', content)
            ...
            if day_match:
                ...
                content = content[len(day_text):].strip()
            
            # Create ISO date
            iso_date = f"1915-{month_num:02d}-{current_day:02d}"
            
            # Create raw date
            day_text = get_chinese_number(current_day) + "日"
            date_raw = f"一九一五年{month_name}{day_text}"
    ...
    for month, count in sorted(months_count.items(), key=lambda x: month_map.get(x[0].replace("一九一五年", ""), 0)):
...
def save_common_diary_json(items):
    ...
    os.makedirs("output_1915", exist_ok=True)
    
    # Save as a single JSON file
    output_file = "output_1915/luxun_yimao_diary.json"
    ...
    individual_dir = "output_1915/individual_entries"
...
if __name__ == "__main__":
    print("Starting to scrape Lu Xun's 1915 diary...")
