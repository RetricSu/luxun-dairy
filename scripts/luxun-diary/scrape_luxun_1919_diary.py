...
# URL of the Lu Xun diary for 1919
URL = "https://zh.wikisource.org/zh-hans/%E9%AD%AF%E8%BF%85%E6%97%A5%E8%A8%98/%E5%B7%B1%E6%9C%AA%E6%97%A5%E8%A8%98"
...
def scrape_luxun_diary():
    ...
    # Find all month headings (h3 in this case, based on webpage structure)
    month_headings = soup.find_all("h3")
    print(f"Found {len(month_headings)} month headings")
    
    # Track all month entries separately
    all_months_entries = {
        "正月": [], "二月": [], "三月": [], "四月": [], "五月": [], "六月": [],
        "七月": [], "八月": [], "九月": [], "十月": [], "十一月": [], "十二月": []
    }
    ...
    for heading in month_headings:
        ...
        # Get all the content elements after this heading until the next heading
        content_elements = []
        ...
        while next_sibling:
            ...
            elif next_sibling.name == 'div' and next_sibling.find('h3'):
                break
            next_sibling = next_sibling.next_sibling
        
        print(f"  Found {len(content_elements)} content blocks")
        ...
        for elem in content_elements:
            ...
            # Special case: if content is just "九月", this marks the September section
            if content == "九月" and month_name == "八月":
                print("  Detected September marker")
                current_month = "九月"
                current_month_num = 9
                current_day = 0  # Will be incremented later
                continue
            ...
            if day_match:
                ...
                # No day specified, increment current day
                current_day += 1
            
            # Create ISO date
            iso_date = f"1919-{current_month_num:02d}-{current_day:02d}"
            
            # Create raw date
            day_text = get_chinese_number(current_day) + "日"
            date_raw = f"一九一九年{current_month}{day_text}"
            ...
            # Add to the appropriate month's entries
            all_months_entries[current_month].append(diary_item)
        
        print(f"  Processed {len(all_months_entries[month_name])} entries for {month_name}")
        if month_name == "八月" and len(all_months_entries["九月"]) > 0:
            print(f"  Also found {len(all_months_entries['九月'])} entries for 九月")
    
    # Combine all months' entries
    for month in month_map.keys():
        all_diary_items.extend(all_months_entries[month])
    
    print(f"Successfully processed {len(all_diary_items)} diary entries")
    
    # Group entries by month for reporting
    months_count = {}
    for month, entries in all_months_entries.items():
        if entries:
            months_count[f"一九一九年{month}"] = len(entries)
    
    for month, count in sorted(months_count.items(), key=lambda x: month_map.get(x[0].replace("一九一九年", ""), 0)):
...
def save_common_diary_json(items):
    ...
    os.makedirs("output_1919", exist_ok=True)
    
    # Save as a single JSON file
    output_file = "output_1919/luxun_jiwei_diary.json"
    ...
    individual_dir = "output_1919/individual_entries"
...
if __name__ == "__main__":
    print("Starting to scrape Lu Xun's 1919 diary...")
